...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'border-b' : ''}`}
      style={scrolled ? { background:'rgba(8,14,20,0.85)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderColor:'rgba(255,255,255,0.07)' } : {}}>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => handleNav('#hero')} className="font-black text-xl text-white hover:opacity-90 transition-opacity">
          ...
          {NAV_LINKS.map(link => {
            ...
            return (
              <button
                ...
                style={isActive ? { color:'#34d399', background:'rgba(16,185,129,0.1)' } : { color:'#9ca3af' }}
              >
                {link.label}
              </button>
            );
        ...
        {/* Mobile hamburger */}
        <button onClick={() => setMobileOpen(v => !v)} className="md:hidden w-9 h-9 rounded-lg flex items-center justify-center transition-all" style={{ color:'#9ca3af', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)' }}>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            style={{ background:'rgba(8,14,20,0.96)', backdropFilter:'blur(20px)', borderColor:'rgba(255,255,255,0.07)' }}
          >
            <div className="px-4 py-4 flex flex-col gap-1">
              ...
              <div className="flex gap-2 mt-2 pt-2 border-t" style={{ borderColor:'rgba(255,255,255,0.07)' }}>
